...
class CudaCodegen:
    ...
    def gen_server_inc(self) -> str:
        ...
        Handlers are now generated as cuda_gen namespace implementations
        (matching the RPC binding declarations), so zpp::bits server dispatch
        can call them directly via bind<&cuda_gen::func_name, index>.
        """
        lines = []
        lines.append("// AUTO-GENERATED by codegen/codegen.py — DO NOT EDIT")
        lines.append(f"// Contains {self.total_active()} CUDA Driver API server handler bodies")
        lines.append("// These are IMPLEMENTATIONS of the function declarations in gen_api.hpp")
        lines.append("// They must be included in a file that has access to real CUDA headers")
        lines.append("// and the server's global state (g_vhandles, g_host_mirror, etc.)")
        lines.append("")
        lines.append("namespace cuda_gen {")
        lines.append("")
        ...
        for func in self.active_functions:
            if func.get("local_only", False) or func.get("stub_return") is not None:
                continue  # No server handler needed (local_only = client-side, stub = no RPC)
    ...
    def _gen_server_handler(self, func: dict) -> str:
        ...
        lines.append(f"{struct_name} {rpc_name}({', '.join(handler_params)}) {{")
