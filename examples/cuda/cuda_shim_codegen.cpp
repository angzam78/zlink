// zlink/examples/cuda/cuda_shim_codegen.cpp
//
// CUDA Driver API shim — CODEGEN-BASED, comprehensive coverage.
//
...
// All 269+ cu* function calls are intercepted and forwarded via zlink RPC
// to a remote GPU server running cuda_server_codegen.
//
// KEY DESIGN:
...
static void shim_init() {
    std::cerr << "zlink CUDA codegen shim loaded\n"
...
static void shim_fini() {
    if (g_transport && g_transport->is_connected()) {
        g_transport->close();
        std::cerr << "zlink codegen shim: disconnected\n";
    }
}
